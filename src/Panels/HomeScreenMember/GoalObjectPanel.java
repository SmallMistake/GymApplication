...
public class GoalObjectPanel extends JPanel {
	...
	public GoalObjectPanel(String username) {
		...
		ResultSet rs = database.sendStatement("SELECT `member`.`memberID` FROM `member` WHERE `member`.`Username` = \"" + username + "\"");
		try {
			while(rs.next()) {
	...
	public void updateMainGoalPanel() {
		goals.clear();
		goalsCounter = 0;
		ResultSet rs = database.sendStatement("SELECT `goals`.`description` FROM `goals` INNER JOIN `member` ON `goals`.`memberID` = `member`.`memberID` WHERE `member`.`Username` = \"" + username + "\"");
		try {
	...
	public void updateSubGoalPanel(String description) {
		...
		ResultSet rs = database.sendStatement("SELECT `goals`.`currentValue`, `goals`.`goalValue`, `member`.`memberID` FROM `goals` INNER JOIN `member` ON `goals`.`memberID` = `member`.`memberID` WHERE `member`.`Username` = \"" + username + "\" AND `goals`.`description` = \"" + description + "\"");
		try {
			while(rs.next()) {
